...
<Aside type="note">
Password auth is SQLite-only. `ssh_config` entries always use key auth.
</Aside>
...
4. **Optional: organize with groups**:
   ...
   Groups must exist before `dssh add --group`. See the [grouping guide](/guides/grouping/) for TUI workflows, assignment, and source-scope details.
...
<Aside type="caution">
Forget the master passphrase and the encrypted passwords are unrecoverable. `dssh reset` deletes the SQLite database, including SQLite connections, groups, memberships, and settings. `ssh_config` entries are untouched.
</Aside>
...
Four tabs — **Create**, **Connect**, **Edit**, and **Delete** — share a fixed two-pane layout. Use `Tab` / `Shift+Tab` to change pane focus and `←` / `→` to change tabs where allowed.
...
Select `(No Groups)` to remove the group filter and show all connections. See the [TUI keys reference](/reference/tui-keys/) for group management, complete controls, and context-specific behavior.

## Where to next

<CardGrid>
  <LinkCard title="Grouping" href="/guides/grouping/" description="Organize, assign, and filter connections in the TUI or CLI." />
